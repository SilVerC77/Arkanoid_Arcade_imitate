...
//許弘毅追加20211020
//_________________________________________________________________________________________________________
void CFont::render(XMFLOAT4X4 matView_, XMFLOAT4X4 matProj_, string str_, size_t texelIndex_, XMFLOAT4 diffuse_) {
	const char* single = str_.data();
	int value = 0;
	for (int i0 = digits - 1; i0 >= 0; i0--) {
		value = single[i0] - 65;
		int index = digits - 1 - i0;
		objects[index]->setAnimation(value);
		objects[index]->render(matView_, matProj_);
	}
}
//_________________________________________________________________________________________________________
void CFont::release()
